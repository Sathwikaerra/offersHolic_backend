...
const verifyOtpCore = async (email, mobileNo, otp) => {
  try {
    ...
    if (email) {
      ...
      const url = `https://2factor.in/API/V1/28ddde3b-8f14-11ef-8b17-0200cd936042/SMS/VERIFY3/${mobileNo}/${otp}`;
    ...
    if (email) {
      ...
      const databaseOtp = await UserOTP.find({ email: value.toLowerCase().trim()  });
