...
export const createBusinessProfile = async (req, res) => {
  try {
    const { name, description, category,  GSTIN, logo, businessPictureGallery, website, phoneNumber, email, socialMedia, addressId, addressDetails } = req.body;
    const owner = getUserIdFromToken(req);
 
    if (!GSTIN || GSTIN === '') {
      return res.status(401).json({ msg: 'GSTIN is required' });
    }
    ...
    try {
      ...
      gstData = await validateGSTWithPrimaryAPI(GSTIN);
    } catch {
      // If the primary API fails, use the secondary API
      gstData = await validateGSTWithSecondaryAPI(GSTIN);
    }
    console.log(gstData)
    if (!gstData.status) {
      return res.status(401).json({ msg: 'GSTIN is invalid' });
    }
    console.log('GST Data:', gstData);
    // const gstName = gstData.data?.legalName ===  ? gstData.data.legalName || gstData.data.tradeName : gstData.name || gstData.tradename;
function normalize(str) {
  return (str || "").toString().trim().toLowerCase();
}

const matchingGSTName =
  normalize(gstData?.data?.legalName) === normalize(name) ||
  normalize(gstData?.result?.source_output?.trade_name) === normalize(name);    // if (!gstName || gstData.data.legalName.toLowerCase() || gstData.data.tradeName.toLowerCase() !== name.toLowerCase()) {
    //make sure the name should match atleast the legal name or trade name
    if (!matchingGSTName) {
      return res.status(401).json({ msg: 'GSTIN name does not match' });
    }


    // Check if addressId is provided and valid
    if (addressId) {
      ...
      // If addressId is not provided, create a new Address document
    ...
    const newBusinessProfile = new BusinessProfile({
      ...
      description,
      category,
      location: locationAddress ? locationAddress._id : null, // Assign the address _id to location
      logo,
    ...
    // Add the created business profile to the individual user's profile
    const updatedUser = await User.findOneAndUpdate(
      { _id: owner },
      { $push: { 'business.businessProfiles': { business: savedBusinessProfile._id } } },
      { new: true }
    );
...
export const getAllCurrentUserBusinessProfile = async (req, res) => {
  try {
    ...
    const businessProfiles = await BusinessProfile.find({ User: userId })
      .populate("category")
      .populate("location")
      // .populate({ path: "ads", populate: { path: "adType.typeRef" } });
